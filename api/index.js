...
// /metrics — базовые метрики по БД в формате Prometheus
app.get('/metrics', async (_req, res) => {
  try {
    const [{ c: in_total }] = await db('incoming_tx').count('* as c');
    const [{ c: in_confirmed }] = await db('incoming_tx')
      .where({ status: 'confirmed' }).count('* as c');

    const [{ c: out_queued }] = await db('outgoing_tx')
      .where({ status: 'queued' }).count('* as c');
    const [{ c: out_signed }] = await db('outgoing_tx')
      .where({ status: 'signed' }).count('* as c');
    const [{ c: out_broadcast }] = await db('outgoing_tx')
      .where({ status: 'broadcast' }).count('* as c');
    const [{ c: out_confirmed }] = await db('outgoing_tx')
      .where({ status: 'confirmed' }).count('* as c');
